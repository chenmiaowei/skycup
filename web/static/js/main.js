$(function(){
	...
	$(".banner li").each(function(){
		...
		if(img.complete || img.width){
			...
			if(n == $(".banner li").length){
				$(".banner .gray-bg").attr("style","background:url("+src+") no-repeat center");
			}
			n -- ;
			if(n == 0){
				banner();
	...
	$(window).scroll(function(event) {
		if($(window).scrollTop() >= $(window).height() && isIdxBus){
			isIdxBus = false;
			bus($(".bus .silder-format").length, 3, $(".bus .arrow-box"), $(".bus svg"), $(".bus .arrow-pre-one"), $(".bus"), 800)
		}

		if($(window).scrollTop() >= $(window).height() + 480 && isIdxInvest){
			isIdxInvest = false;
			bus($(".invest .silder-format").length, 5, $(".invest .arrow-box"), $(".invest svg"), $(".invest .arrow-pre-one"), $(".invest"), 800)
		}
	})
	//end bus

	$(".invest-btn a").click(function(event) {
