...
define(['jquery', 'adminer/js/ui', 'template', 'tools',
    ...
    function($, ui, template, tools) {
            ...
            $.ajax({
                ...
                .done(function(data) {
                    ...
                    if (data.status + 0 === 0) {
                        ...
                        var input_name = _t.parents(
                            '.js-img-list-f').attr('id').replace(
                            'js-', '').replace('-show', '');
                        _t.remove();
                        // 整理界面
                        media.ui_del(id, input_name);
                        // 额外删除的效果的处理
                        if (typeof media.del_done !== 'undefined' &&
                            media.del_done instanceof Function) {
        ...
        media.ui_del = function(id, input_name) {
            // 对单个存储的单独处理
            if ($('input[name=' + input_name + ']').attr('data-more') -
                0 ===
                0) {
                if (id == $('input[name=' + input_name + ']').val()) {
                    $('input[name=' + input_name + ']').val('');
                    $('input[name=' + input_name + ']').next(
                ...
                // 重新裝弹
                var fids = '';
                $('#js-' + input_name + '-show div[data-id]').each(
                    function(index, el) {
                        if (index === 0) {
                            $('input[name=' + input_name + ']').next(
                                'input[name=thumb]').val($(el).attr(
                                'data-thumb'));
        ...
        media.done = function(tdata, input_name) {
            ...
            if ($('input[name=' + input_name + ']').attr('data-more') -
                ...
                $('input[name=' + input_name + ']').next().val(tdata.list[
                    0].thumb);
            } else {
                // 多个
        ...
        media.show = function(tdata, input_name) {
            if (tdata && typeof tdata != 'undefined' && !tools.isEmpty(
                    tdata)) {
                var data; // 数据
                var html; // 模板容器
        ...
        media.get_day_list = function(day, domf) {
            $.ajax({
                ...
                .done(function(data, textStatus) {
                    ...
                    if (data.status + 0 === 0) {
                        ...
                        if (tdata instanceof Array) {
                            ...
                            html = template.render('tpl-photo-show',
                                data);
                        }
                    }
                    $('#' + domf).html(html);
                ...
                .fail(function(XMLHttpRequest, textStatus, errorThrown) {
                    ...
                    $('#' + domf).after(ui.alert('访问数据列表错误 ：<br />' +
                        m));
                });
        };
        ...
        media.init = function(upload_done, del_done) {
                ...
                $(e).fileupload({
                        ...
                        fail: function(XMLHttpRequest,
                            ...
                            if (list[0].error ===
                                ...
                                $('#upload-modal').delay(
                                    1000).hide(500); //.find('.progress .progress-bar').delay(3500).css('width','0%');
                                ...
                                media.done(tdata,
                                    input_name);
                                // 上传后额外UI输出执行函数
                                if (typeof upload_done !=
                                    'undefined' &&
                                    upload_done instanceof Function
                                ) {
                                    upload_done(tdata,
                                        input_name);
                                }
                        ...
                        progressall: function(e, data) {
                            ...
                            var progress = parseInt(
                                ...
                                100, 10);
                            $(
                                '#upload-modal .progress .progress-bar'
                            ).css('width',
                                progress +
                                '%');
                    ...
                    .addClass($.support.fileInput ? undefined :
                        'disabled');
            });

            // 关闭
            $('#upload-modal > .upload-header > a.upload-close').on(
                'click',
                function(e) {
                    e.preventDefault();
                    $('#upload-modal').hide();
            ...
            // 对 daylist 删除
            $('#upload-modal').delegate('li a[data-del]', 'click',
                function(e) {
                    e.preventDefault();
            ...
            // 显示区域删除图片
            $('.js-img-list-f').delegate('.js-goal a[data-del]',
                'click',
                function(e) {
                    e.preventDefault();
            ...
            // 图片seo
            $('.js-img-list-f').delegate('.js-goal a[data-seo]',
                'click',
                function(e) {
                    e.preventDefault();
