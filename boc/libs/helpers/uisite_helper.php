...
//根据ID获取市名称---经销商lj
function get_title($id,$table='city')
{
	$result = get_Simplify($table,'title',array('id'=>$id));
	if(!empty($result))
	{
		return $result->title;
	} else {
...
//根据ID获取市名称---经销商lj
function get_type($id,$table='upload')
{
	$result = get_Simplify($table,'type',array('id'=>$id));
	if(!empty($result))
	{
		return $result->type;
	} else {
